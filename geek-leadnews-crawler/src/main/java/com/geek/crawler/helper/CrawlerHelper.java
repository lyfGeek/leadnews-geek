...
public class CrawlerHelper {
     ...
     * 数据校验辅助类。
     */
    private IDataValidateCallBack dataValidateCallBack;

    public IDataValidateCallBack getDataValidateCallBack() {
        return dataValidateCallBack;
    }

    public void setDataValidateCallBack(IDataValidateCallBack dataValidateCallBack) {
    ...
    public boolean requestValidation(Page page) {
        ...
        boolean flag = false;
        IDataValidateCallBack dataValidateCallBack = getDataValidateCallBack();
    ...
    public boolean requestValidation(CrawlerHtml crawlerHtml) {
        ...
        if (null != crawlerHtml && StringUtils.isNotEmpty(crawlerHtml.getHtml())) {
            IDataValidateCallBack dataValidateCallBack = getDataValidateCallBack();
            if (null != dataValidateCallBack) {
                flag = dataValidateCallBack.validate(crawlerHtml.getHtml());
                log.info("校验数据状态：flag：{}", flag);
            }
        }
