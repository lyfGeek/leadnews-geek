package com.geek.crawler.process.download;
...
/**
 * 代理模式下进行 Http 方式的下载。
 * ProxyHttpClientDownloader 类是使用代理方式进行数据页面数据下载，
 * 但是不满足需求做了一些重试的定制，是对 HttpClientDownloader 类的一些扩展，
 * 是从 https:// gitee.com/flashsword20/webmagic/blob/master/webmagic-core/src/main/java/us/codecraft/webmagic/downloader/HttpClientDownloader.java
 * 获取源码后编辑的来的。
 * <p>
...
public class ProxyHttpClientDownloader extends AbstractDownloader implements IProcessFlow {
    ...
    public CrawlerHtml proxySeleniumDownloadRetry(Page page) {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            log.info("尝试使用 selenium 下载数据第 {} 次，url：{}，代理：{}", i + 1, page.getUrl(), proxy);
            crawlerHtml = seleniumClient.getCrawlerHtml(page.getUrl().toString(), proxy, cookieHelper.getCookieName());
            log.info("尝试使用 selenium 下载数据第 {} 次完成，代理：{}，url：{}，耗时：{}", i, proxy, page.getUrl(), System.currentTimeMillis() - currentTime);
