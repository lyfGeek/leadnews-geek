...
public class SeleniumClient {
    ...
    private SeleniumDriver initChromeDriver(Proxy proxy) {
        ...
        log.info("创建 Chrome 驱动完成，耗时：" + (System.currentTimeMillis() - currentTime));
    ...
    private void handel(String url, Proxy proxy, ChromeCallback chromeCallback) {
        ...
        if (StringUtils.isNotEmpty(url) && null != chromeCallback) {
            try {
                ...
                log.info("chrome 调用失败：" + e.getMessage());
            } finally {
                log.info("关闭 chrome 驱动。");
    ...
    /**
     * 获取 Cookie 的延时方法。
     * 因为浏览器打开页面有可能 js 还没有执行完成，获取的数据是不准确定，通过有没有写入一些特殊的 cookie 来判断页面是否已经加载完成。
     * 如果没有这个 cookie 则页面一直重复循环，一直等到达到超时时间。
     *
     * @param driver
     * @param cookieName
     */
    private List<CrawlerCookie> delayed(final WebDriver driver, final String cookieName) {
        Object value = DelayedUtils.delayed(new IDelayedCallBack() {
            ...
            /**
             * 每次循环获取需要睡眠的时间。
             * 防止一直获取 cpu 资源耗费的太多，默认 1 秒重复获取一次。
             * @return
             */
            ...
            /**
             * 配置的超时时间，防止页面没有找到 cookie 就一直在循环，没有退出。
