...
public class SimHashUtils {
    ...
    private static String cleanResume(String content) {
        // 若输入为 HTML，下面会过滤掉所有的 HTML 的 tag。
    ...
    private static BigInteger simHash(String token, int hashbits) {
        ...
        weightOfNature.put("n", 2);// 给名词的权重是 2。
        ...
        for (Term term : termList) {
            ...
            // 2、将每一个分词 hash 为一组固定长度的数列。比如 64bit 的一个整数。
            BigInteger t = hash(word, hashbits);
            for (int i = 0; i < hashbits; i++) {
                BigInteger bitmask = new BigInteger("1").shiftLeft(i);
                // 3、建立一个长度为64的整数数组（假设要生成 64 位的数字指纹，也可以是其它数字）。
                // 对每一个分词 hash 后的数列进行判断，如果是 1000...1，那么数组的第一位和末尾一位加1，
                // 中间的 62 位减一。也就是说，逢 1 加 1，逢 0 减 1。一直到把所有的分词 hash 数列全部判断完毕。
                int weight = 1;//添加权重。
    ...
    private static BigInteger hash(String source, int hashbits) {
        if (source == null || source.length() == 0) {
            ...
            /**
             * 当 source 的长度过短，会导致 hash 算法失效，因此需要对过短的词补偿。
             */
