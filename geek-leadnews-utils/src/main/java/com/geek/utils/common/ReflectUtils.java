...
public class ReflectUtils {
     ...
     * @param value
     */
    public static void setProperties(Object bean, String key, Object value) {
     ...
     */
    public static void setProperties(Object bean, String key, Object value, boolean skipExist) {
        if (null != bean && StringUtils.isNotEmpty(key)) {
            if (skipExist) {
                Object propValue = getProperties(bean, key);
                if (null == propValue) {
                    setProperties(bean, key, value);
                }
            } else {
                setProperties(bean, key, value);
            }
    ...
    /**
     * 通过反射将 map 的 key value 映射到实体类中。
     *
     * @param bean
     * @param skipExist 是否跳过已存在的属性。
     */
    public static void setProperties(Object bean, Map<String, Object> parameterMap, boolean skipExist) {
        if (null != bean && null != parameterMap && !parameterMap.isEmpty()) {
            for (Map.Entry<String, Object> entry : parameterMap.entrySet()) {
                setProperties(bean, entry.getKey(), entry.getValue());
            }
        }
    }
     ...
     * @return
     */
    public static Object getProperties(Object bean, String key) {
        Object value = null;
        if (null != bean && StringUtils.isNotEmpty(key)) {
            PropertyDescriptor[] descriptor = getPropertyDescriptorArray(bean);
            PropertyDescriptor propertyDescriptor = getPropertyDescriptor(descriptor, key);
